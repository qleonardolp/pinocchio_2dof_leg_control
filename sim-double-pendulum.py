...
sim_steps = math.floor(sim_duration/dt)
...
# Controller (PD):
Kp = np.ones(model.nv)*100
Kd = np.ones(model.nv)*25
# print(Kp)
ctrl_type = 'pdff'

q_des = np.array([math.pi*(178/180), math.pi*(90/180)])
dq_des = np.zeros(model.nv)
# q = pin.randomConfiguration(model)
# Initial states, q0, dq0, ddq0
q  = np.array([math.pi*(135/180),
               math.pi*(15/180)])
dq = np.zeros(model.nv)

t = 0.00
data_sim = model.createData()
q_log   = np.zeros([sim_steps, 1 + model.nq])
dq_log  = q_log.copy()
ddq_log = q_log.copy()
...
for k in range(sim_steps):
    ...
    if ctrl_type == 'pd':
        tau_control = Kp * (q_des - q) + Kd * (dq_des - dq)
    if ctrl_type == 'pdff':
        tau_control = Kp * (q_des - q) + Kd * (dq_des - dq) + np.array([25, 12])
    if ctrl_type == 'id':
        tau_control = Kp * (q_des - q) + Kd * (dq_des - dq) #+ pin.computeCoriolisMatrix(model,data_sim,q,dq)
    ...
    # Log variables
    q_log[k]   = [t, math.degrees(q[0]), math.degrees(q[1])]
    dq_log[k]  = [t, math.degrees(dq[0]), math.degrees(dq[1])]
    ddq_log[k] = [t, math.degrees(ddq[0]), math.degrees(ddq[1])]
